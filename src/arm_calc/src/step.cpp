...
static inline double get_quintic_value(const ArmStep::QuinticLineParam_t& line, const double time) {
...
static inline double get_quintic_dt(const ArmStep::QuinticLineParam_t& line, const double time) {
    return line.b + 2.0f * line.c * time + 3.0f * line.d * time * time + 4.0f * line.e * time * time * time
         + 5.0f * line.f * time * time * time * time;
}

static inline double get_quintic_dtdt(const ArmStep::QuinticLineParam_t& line, const double time) {
...
void ArmStep::update_support_trajectory(const Vector3D& cur_pos, const Vector3D final_pos, double time) {
    support_trajectory.time = time;                        // 记录一个步态相位的时间
...
void ArmStep::update_flight_trajectory(
...
std::tuple<Vector3D, Vector3D, Vector3D> ArmStep::get_target(double time, bool& success) {
    Vector3D pos;
    Vector3D vel;
    Vector3D acc;
